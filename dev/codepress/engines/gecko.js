...
CodePress = {
	...
	syntaxHighlight : function(flag) {
		...
		x = x.replace(/\t/g,'<kbd>\t</kbd>'); // --- added by T.Otani
		x = x.replace(/\n/g,'<br>');
...
/*
		// Original Code (Cannot input Tab)
...
*/
		...
		// @author Asad Khan <asadkn@gmail.com>
		var noInsertion = 1;
		for (var i=0; i<snippets.length; i++) {
			if(snippets[i].input == trigger) {
				noInsertion = 0;
		...
		if (noInsertion == 1) {
			...
			if (range.commonAncestorContainer.getElementsByTagName) {
				...
				for (var i = 0; i < childNodes.length; i++) {
					...
					&& srcRange.compareBoundaryPoints(Range.END_TO_END, range) <= 0) {
						if (lastBr) {
